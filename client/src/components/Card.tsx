  ...
  return (
    <div className="shrink-0 w-[142px] md:w-[172px] cursor-pointer scroll-snap-start group" onClick={() => nav(`/title/${title.id}`)}>
      ...
      >
        ...
        {/* Rank number */}
        {rank && (
          <span className="absolute top-1.5 left-2 z-10 font-serif font-bold text-[34px] text-transparent leading-none"
      ...
      <div className="font-mono text-[10.5px] text-ink-faint flex items-center gap-1 truncate">
        {title.year}
        {genre && <><span className="text-line">·</span><span className="truncate">{genre}</span></>}
      </div>
    </div>
  );
}

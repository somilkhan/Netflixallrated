...
export function InlineLoader({
  ...
  return (
    ...
    >
      ...
      {label && <p className="text-xs tracking-wide text-ink-faint animate-[glPulse_1.8s_ease-in-out_infinite]">{label}</p>}
    </div>
  );
}
...
export default function GlassLoader({
  ...
  return (
    <div
      className={`fixed inset-0 z-[999] flex flex-col items-center justify-center gap-5 bg-page/90 md: transition-opacity duration-300 ease-out ${visible ? 'opacity-100' : 'opacity-0'}`}
      role="status"
      aria-live="polite"
      aria-label={label || 'Loading'}
    >
      <LoaderMark size={72} />
      {label && <p className="font-mono text-sm tracking-wide text-ink-dim animate-[glPulse_1.8s_ease-in-out_infinite]">{label}</p>}
    </div>
  );
}

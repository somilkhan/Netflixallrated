  ...
  return (
    ...
    >
      <div className="relative w-full aspect-[2/3] rounded-lg">
        {/* Poster Link container acting as the direct link trigger to TitleDetail page */}
        <Link
          to={onNavigate ? '#' : `/title/${title.id}`}
          onClick={(e) => {
            if (onNavigate) {
              e.preventDefault();
              handleClick();
          ...
          className="
            block relative w-full h-full rounded-lg overflow-hidden
            bg-[#141414]
        ...
        {/* ── Desktop hover overlay (Sibling of Link container to prevent nesting interactive tags) ──────────────── */}
        <div className="
          hidden md:block
        ...
        {showProgress && progressPct > 0 && (
          <div className="absolute bottom-0 inset-x-0 z-30 h-[4px] bg-white/10 pointer-events-none">

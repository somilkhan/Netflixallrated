...
export default function Admin() {
  ...
  if (!user) return <div className="p-10 text-center text-ink-dim">Admin access required. <button onClick={() => nav('/login')} className="text-maroon-bright">Sign in</button></div>;
  if (user.role !== 'ADMIN') return <div className="p-10 text-center text-ink-dim">This page is for admins only.</div>;
  ...
  const importTitle = async (r: TmdbSearchResult) => {
    ...
    try {
      ...
      setMessage(err.message === 'Already imported' ? `"${r.name}" is already in the catalog.` : `Failed to import "${r.name}".`);
    } finally {
      setImportingId(null);
    }
  };
  ...
  return (
    <div className="px-5 py-8 max-w-2xl mx-auto">
      <h1 className="font-serif text-2xl font-semibold mb-2">Add Title</h1>
      <p className="text-ink-dim text-sm mb-6">Search TMDB and import a title — poster, synopsis, genres, runtime and trailer are pulled in automatically.</p>
      ...
      <form onSubmit={runSearch} className="flex gap-2 mb-6">
        ...
        />
        <button type="submit" disabled={searching} className="px-5 py-3 bg-maroon-bright text-white rounded-lg font-semibold hover:bg-maroon transition-colors disabled:opacity-50">
          {searching ? 'Searching…' : 'Search'}
        </button>
      </form>

      {message && <div className="mb-4 text-sm font-mono text-amber">{message}</div>}
      ...
      <div className="space-y-3">
        {results.map(r => (
          <div key={r.tmdbId} className="flex gap-3 bg-surface border border-line rounded-lg p-3 items-center">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="font-mono text-[10.5px] text-ink-faint">{r.year || '—'} · {r.mediaType === 'movie' ? 'Movie' : 'Series'}</div>
        ...
        {!searching && results.length === 0 && query && <div className="text-ink-faint text-sm font-mono">No results yet — try searching above.</div>}
      </div>
    </div>
  );
}

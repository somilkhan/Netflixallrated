...
function BackButton() {
  ...
  return (
    <button
      onClick={() => navigate(-1)}
      className="flex items-center gap-1.5 text-base text-ink-secondary
    ...
    <div className="flex gap-1 mb-6 overflow-x-auto scrollbar-hide">
      {options.map((key) => (
        <button
          ...
          className={`shrink-0 text-base font-medium px-4 py-2 rounded-full border transition-all duration-200
            ${active === key
              ? "bg-white text-black border-white"
              : "bg-transparent border-border-light text-ink-secondary hover:text-white hover:border-white/25"}`}
        >
          {TAB_LABEL[key]}
        </button>
      ))}
  ...
  return (
    <>
      ...
      {loadState === "done" && !hasNext && items.length > 0 && (
        <p className="text-sm text-white/25 text-center py-6 pb-24">
...
function DetailShell({
  ...
  return (
    <div className="min-h-screen px-5 pt-10 pb-28" style={{ background: '#000000' }}>
      <BackButton />
      <h1 className="text-7xl md:text-[42px] font-bold leading-tight text-white mb-1 capitalize tracking-tight">
        {title}
      </h1>
      {subtitle && (
        <p className="text-base text-ink-secondary mb-6">{subtitle}</p>
      )}
      {children}
    </div>
  );

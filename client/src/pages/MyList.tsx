  ...
  return (
    ...
    >
      <button
        ...
        className="absolute right-0 top-0 bottom-8 z-0 flex w-[76px] items-center justify-center rounded-lg bg-primary/80 text-white md:hidden"
      >
        <X size={18} />
      </button>
      <div
      ...
      >
        ...
        <button
          ...
          aria-label={`Remove ${item.title?.name ?? 'Untitled'} from My List`}
          className="absolute right-2 top-2 z-30 hidden h-7 w-7 items-center justify-center rounded-full border border-white/15 bg-black/75 text-white/80 shadow-lg transition-colors hover:bg-primary hover:text-white md:flex"
        >
          <X size={14} />
        </button>
      </div>
  ...
  if (!user) {
    return (
      <div className="min-h-[60vh] px-6 flex flex-col items-center justify-center gap-4 text-center">
        <Bookmark size={36} className="text-ink-disabled" />
        <div>
          <h1 className="text-xl font-semibold text-white">Your list is waiting</h1>
          <p className="mt-2 text-sm text-ink-secondary">Sign in to save movies and shows for later.</p>
        </div>
        <button type="button" onClick={() => nav('/login')} className="rounded-lg bg-white px-5 py-2.5 text-sm font-semibold text-black hover:bg-white/90">
          Sign In
        </button>
      </div>
    );
    ...
    <div className="mx-auto max-w-[1400px] px-5 pt-[88px] pb-32">
      <div className="mb-7 flex items-end justify-between gap-4">
        <div>
          <div className="mb-2 flex items-center gap-2 text-ink-tertiary">
            <Bookmark size={17} />
            <span className="text-xs uppercase tracking-[0.18em]">Collection</span>
          </div>
        ...
        <button type="button" onClick={() => nav('/browse')} className="hidden items-center gap-1 rounded-full border border-border-light px-4 py-2 text-sm text-white/55 transition-colors hover:border-white/25 hover:text-white sm:flex">
          ...
          {statuses.map(status => (
            <button
              ...
              className={`whitespace-nowrap rounded-full border px-3.5 py-2 text-xs transition-colors ${
                active === status ? 'border-white/25 bg-overlay-medium text-white' : 'border-border-light text-ink-secondary hover:text-white/75'
              }`}
            >
      ...
      {loading && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {Array.from({ length: 12 }).map((_, index) => <div key={index} className="aspect-[2/3] animate-pulse rounded-lg bg-white/[0.05]" />)}
        </div>
      )}

      {!loading && error && (
        <div className="py-20 text-center">
          <p className="mb-4 text-sm text-ink-secondary">Couldn’t load your list.</p>
          <button type="button" onClick={load} className="rounded-lg border border-white/15 px-4 py-2 text-sm text-white/65 hover:border-border-hover hover:text-white">Try again</button>
        </div>
      )}
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="flex min-h-[420px] flex-col items-center justify-center rounded-xl border border-white/[0.07] bg-white/[0.02] px-6 text-center">
          ...
          <p className="mt-2 max-w-sm text-sm leading-6 text-ink-secondary">Save movies and shows you want to watch later. They’ll appear here.</p>
          <button type="button" onClick={() => nav('/browse')} className="mt-6 rounded-lg bg-white px-5 py-2.5 text-sm font-semibold text-black hover:bg-white/90">Browse</button>
        </div>
      )}

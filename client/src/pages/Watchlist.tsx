        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {filtered.map((item: any) => (
            <div key={item.id} className="cursor-pointer group" onClick={() => nav(`/title/${item.title.id}`)}>
              <div className="aspect-[2/3] rounded-xl border border-line overflow-hidden mb-2 transition-[border-color] duration-200 group-hover:border-white/40 bg-cover bg-center" style={{
